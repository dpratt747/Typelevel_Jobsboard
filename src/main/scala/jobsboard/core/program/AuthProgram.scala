    ...
    usersRepositoryAlg.find(user.email).flatMap {
      ...
      case None =>
        for {
          ...
          userItem = User(user.email, Password(hashedPassword), user.firstName, user.lastName, user.company, Role.RECRUITER)
...
object AuthProgram {
  def make[F[_] : Logger : Async](
    ...
    val idStore: IdentityStore[F, Email, User] = (email: Email) => OptionT(usersRepositoryAlg.find(email))
