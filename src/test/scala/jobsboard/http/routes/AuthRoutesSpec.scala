...
import jobsboard.http.routes.{JobRoutes, RequestUtil}
...
class AuthRoutesSpec
  ...
  describe("AuthRoutes") {
      ...
      forAll(emailGen, passwordGen) { case (email, password) =>
        ...
        val mockAuthProgram = new AuthProgramAlg[IO] {
          ...
          override def login(
              email: Email,
              password: Password
          ): IO[Option[AugmentedJWT[HMACSHA256, Email]]] = IO.pure(None)
        ...
        (for {
          ...
          request = Request[IO](method = POST, uri = uri"/auth/login").withEntity(
      ...
      forAll(emailGen, passwordGen) { case (email, password) =>
        ...
        val mockAuthProgram = new AuthProgramAlg[IO] {
          ...
          override def login(
              email: Email,
              password: Password
          ): IO[Option[AugmentedJWT[HMACSHA256, Email]]] = mockedAuthenticator.create(email).option
      ...
      forAll(newUserInfoGen) { case (newUser) =>
        ...
        val mockAuthProgram = new AuthProgramAlg[IO] {
          ...
          override def login(
              email: Email,
              password: Password
          ): IO[Option[AugmentedJWT[HMACSHA256, Email]]] = ???
    ...
    it(
      "[POST] /auth/logout should return 401 - unauthorised when logging out without a JWT token"
    ) {
      forAll(emailGen, passwordGen) { case (email, password) =>
        val mockedAuthenticator: JWTAuthenticator[IO, Email, User, HMACSHA256] = {
          val key                                     = HMACSHA256.unsafeGenerateKey
    ...
    it("[PUT] /auth/user/password should return 200 - Ok if the password is changed") {
      forAll(emailGen, passwordGen, passwordGen, usersGenerators) {
        case (email, password, oldPassword, user) =>
          val mockedAuthenticator: JWTAuthenticator[IO, Email, User, HMACSHA256] = {
            val key                                     = HMACSHA256.unsafeGenerateKey
            val idStore: IdentityStore[IO, Email, User] = (_: Email) => OptionT.some[IO](user)
    ...
    it(
      "[DELETE] /auth/user/<email> should return 401 - Unauthorized if a non admin tries to delete a user"
    ) {
      forAll(usersGenerators, emailGen) { case (user, emailToDelete) =>
        val mockedAuthenticator: JWTAuthenticator[IO, Email, User, HMACSHA256] = {
          val key = HMACSHA256.unsafeGenerateKey
          val idStore: IdentityStore[IO, Email, User] =
            (_: Email) => OptionT.some[IO](user.copy(role = Role.RECRUITER))
    ...
    it("[DELETE] /auth/user/<email> should return 200 - Ok if an admin tries to delete a user") {
      forAll(usersGenerators, emailGen) { case (user, emailToDelete) =>
        val mockedAuthenticator: JWTAuthenticator[IO, Email, User, HMACSHA256] = {
          val key = HMACSHA256.unsafeGenerateKey
          val idStore: IdentityStore[IO, Email, User] =
            (_: Email) => OptionT.some[IO](user.copy(role = Role.ADMIN))
        ...
        (for {
          ...
          request = Request[IO](
            method = DELETE,
            uri = Uri.unsafeFromString(s"/auth/user/${emailToDelete.value}")
          )
            .withBearerToken(jwtToken)
          response <- authRoutes.routes.orNotFound.run(request)
        } yield {
